import re  # Per una semplice estrazione di parole chiave
...
italian_stop_words = set (
    ["il", "lo", "la", "i", "gli", "le", "un", "uno", "una", "di", "a", "da", "in", "con", "su", "per", "tra", "fra",
        "e", "o", "ma", "se", "perché", "anche", "come", "che", "cui", "chi", "quale", "quanto", "quando", "dove", "io",
        "tu", "egli", "noi", "voi", "essi", "mio", "tuo", "suo", "nostro", "vostro", "loro", "mi", "ti", "si", "ci",
        "vi", "ne", "lo", "la", "li", "le", "ne", "del", "al", "dal", "nel", "col", "sul", "pel", "tra", "fra", "allo",
        "agli", "alle", "dallo", "dagli", "dalle", "nello", "negli", "nelle", "collo", "cogli", "colle", "sullo",
        "sugli", "sulle", "pello", "pegli", "pelle", "essi", "stesso", "proprio", "ogni", "qualche", "alcuni",
        "nessuno", "tutto", "altro", "molto", "poco", "già", "ancora", "solo", "quasi", "sempre", "mai", "spesso",
        "subito", "dopo", "prima", "durante", "mentre", "adesso", "ora", "qui", "lì", "là", "così", "come", "tale",
        "quale", "circa", "verso", "attraverso", "sopra", "sotto", "davanti", "dietro", "dentro", "fuori", "vicino",
        "lontano", "insieme", "separato", "contro", "senza", "circa", "verso", "attraverso", "non", "sì"])
...
# Nome del file
filename = f"conversazione_punti_chiave_Giovanni_flash_2_0_{datetime.now ().strftime ('%Y%m%d_%H%M%S')}.txt"
...
summary = ""
important_keywords = set ()  # Insieme per memorizzare parole chiave importanti

try:
    with open ("ChatTestWithSummary/" + filename, "w", encoding="utf-8") as f:
        f.write ("[CONTEXT]\n")
        f.write (npc_context + "\n")
        f.write ("[/CONTEXT]\n\n")
        ...
        while True:
            ...
            # Estrazione semplice di parole chiave (potrebbe essere migliorata)
            keywords = [word for word in re.findall (r'\b\w+\b', user_input.lower ()) if word not in italian_stop_words]
            ...
            try:
                ...
                keywords = [word for word in re.findall (r'\b\w+\b', npc_response.lower ()) if
                            word not in italian_stop_words]
                important_keywords.update (keywords)

                # Logica per il riassunto
                if len (conversation_history_for_summary) > 0 and len (conversation_history_for_summary) % (
                    ...
                    summary_prompt = f"""Riassumi la seguente conversazione tra un utente e un gioielliere di nome Giovanni, mantenendo le informazioni chiave, i dettagli importanti per il contesto futuro e menzionando eventuali oggetti o preferenze discusse:
                    {conversation_history_for_summary}
                    Parole chiave importanti finora: {list (important_keywords)}
                    ...
                    if summary_text:
                        ...
                        print ("Riassunto generato e cronologia aggiornata.Tempo di generazione: " + str (datetime.now () - inizioRiassunto) + "\n")
                        f.write ("[CRONOLOGIA AGGIORNATA CON RIASSUNTO"+ str (datetime.now () - inizioRiassunto)+ "]\n")
                    else:
                        print ("Errore nella generazione del riassunto.")
                        f.write ("[ERRORE GENERAZIONE RIASSUNTO]\n")
                print (str (datetime.now () - inizioRisposta) + "\n")
                f.write (str (datetime.now () - inizioRisposta) + "\n")
