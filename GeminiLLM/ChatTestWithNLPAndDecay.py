...
import spacy
import time  # Per simulare il tempo
...
DECAY_RATE = 0.9  # Fattore di decadimento (es. 0.9 significa che il punteggio si riduce del 10% per interazione)
    ...
    with open("ChatTestWithNLPAndDecay/" + filename, "w", encoding="utf-8") as f:
        ...
        while True:
            ...
            try:
                ...
                user_keywords = extract_important_keywords_nlp(user_input)
                ...
                npc_keywords = extract_important_keywords_nlp(npc_response)
                f.write (f"[ESTRAZIONE PAROLE CHIAVE]: {npc_keywords}\n")
                print(f"[ESTRAZIONE PAROLE CHIAVE]: {npc_keywords}\n")
                for keyword in npc_keywords:
                    keyword_scores[keyword] = keyword_scores.get(keyword, 0) + 1

                # Applica il decay a tutti i punteggi delle parole chiave
                for keyword in list(keyword_scores.keys()): # Usa una copia delle chiavi per evitare errori di iterazione
                    keyword_scores[keyword] *= DECAY_RATE
                    if keyword_scores[keyword] < 0.1: # Rimuovi le parole chiave con punteggio molto basso
                        del keyword_scores[keyword]

                print ("Tempo di estrazione parole chiave e decay:", datetime.now () - inizioExtraction)
                f.write(f"Tempo estrazione parole chiave e decay: {datetime.now () - inizioExtraction}\n")
                ...
                if len(conversation_history_for_summary) > 0 and len(conversation_history_for_summary) % (SUMMARY_FREQUENCY * 2) == 0:
                    ...
                    # Ordina le parole chiave per punteggio (in ordine decrescente)
                    inizioRiassunto = datetime.now ()
                    sorted_keywords = sorted(keyword_scores.items(), key=lambda item: item[1], reverse=True)
                    top_keywords = [keyword for keyword, score in sorted_keywords[:15]] # Prendi le prime 10 come esempio
                    summary_prompt = f"""Riassumi la seguente conversazione ..., prestando particolare attenzione agli oggetti, alle preferenze e agli argomenti chiave discussi.
                    Parole chiave rilevanti (con punteggio di rilevanza): {sorted_keywords}
